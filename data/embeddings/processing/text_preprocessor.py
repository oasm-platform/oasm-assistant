"""
Text preprocessing utilities for embedding pipelines.

Overview:
- This module provides light-weight, deterministic text cleanup routines tailored
  for downstream chunking and embedding. It is intentionally conservative to
  avoid altering semantic meaning while fixing common PDF extraction artifacts.

Main steps performed by `TextPreprocessor.preprocess` (configurable via flags):
  1) Unicode normalization (NFC)
  2) Whitespace collapsing (spaces/newlines), preserve paragraph breaks
  3) Control-character stripping (except tabs/newlines)
  4) De-hyphenation across line breaks (e.g., "hy-\nphen" → "hyphen")
  5) Merge hard-wrapped lines into paragraphs where appropriate
  6) Remove repeated headers/footers heuristically
  7) Final trim and newline compaction

Notes:
- The goal is to get stable, clean text for sentence/paragraph chunkers and
  embedding models, not to fully reflow or re-punctuate documents.
- Heuristics are tuned to typical PDF extraction noise and may be adjusted per
  domain.
"""
